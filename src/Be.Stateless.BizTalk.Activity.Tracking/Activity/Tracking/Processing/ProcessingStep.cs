...
namespace Be.Stateless.BizTalk.Activity.Tracking.Processing
{
	public partial class ProcessingStep
	{
		...
		/// tracking context has been created previously, at the start of the orchestration by a call to <see
		/// cref="Process.Initiate(XLANGMessage)"/> or by a call to this method. A new processing step tracking activity is
		/// always started, and it is always linked to the process tracking activity identified in <paramref
		...
		{
			if (string.IsNullOrEmpty(processingStepName)) throw new ArgumentNullException(nameof(processingStepName));
		...
		/// </param>
		/// <remarks>
		/// It will end the processing step activity, setting its <see cref="Status"/> to <see cref="TrackingStatus.Completed"/>,
		...
		{
			if (trackingContext.ProcessingStepActivityId.IsNullOrEmpty())
				throw new ArgumentException("trackingContext.ProcessingStepActivityId is null or empty: processing step tracking has not been initiated.", nameof(trackingContext));
