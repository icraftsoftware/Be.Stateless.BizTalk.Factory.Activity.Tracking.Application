...
{
	/// <summary>
	/// Central file store where message body's payloads are claimed, or stored; that is to say where message body's payloads
	...
	internal class ClaimStore
	{
		...
		/// </summary>
		/// <remarks>
		/// To locate the payload of a business message body, one has to combine this <see cref="CheckOutDirectory"/> with the
		/// claim token's url (see <see cref="MessageBodyCaptureDescriptor.Data"/> when <see
		/// cref="MessageBodyCaptureDescriptor.CaptureMode"/> is <see cref="MessageBodyCaptureMode.Claimed"/>) to get the full
		/// path to the claim.
		/// </remarks>
		...
		/// <summary>
		/// Ascertain, i.e. assess and setup, whether the payload of a message body <paramref name="trackingStream"/> needs to be
		/// captured outside of the BAM monitoring database, that is in the <see cref="ClaimStore"/>, or not.
		/// </summary>
		...
		/// The <see cref="IKernelTransaction"/> factory method whose offspring transaction has to be piggybacked if the message
		/// body's payload needs to be claimed to disk. It can be <c>null</c> if piggybacking is not desired nor possible, like
		/// for instance when calling this method from a send pipeline, as BizTalk does not provide for transaction piggybacking
		/// in send pipelines.
		/// </param>
		/// <returns>
		...
		/// <remarks>
		/// <para>
		/// If the size of the <see cref="TrackingStream"/> does not exceed a 512KB threshold, the <see cref="TrackingStream"/>'s
		/// capture will be setup according to a <see cref="MessageBodyCaptureDescriptor"/> whose <see
		/// cref="MessageBodyCaptureDescriptor.Data"/> will contain a base64-encoding of the compressed stream content.
		/// </para>
		/// <para>
		/// If the size of the <see cref="TrackingStream"/> exceeds the 512KB threshold, the <see cref="TrackingStream"/>'s
		/// capture will be setup according to a <see cref="MessageBodyCaptureDescriptor"/> whose <see
		/// cref="MessageBodyCaptureDescriptor.Data"/> will contain the relative URL that can be used to get back the payload
		/// from the <see cref="ClaimStore"/>.
		/// </para>
		/// <para>
		...
		public virtual void SetupMessageBodyCapture(TrackingStream trackingStream, ActivityTrackingModes trackingModes, Func<IKernelTransaction> transactionFactory)
		{
			...
			if (_logger.IsDebugEnabled)
				_logger.DebugFormat(
					...
					Convert.ToString(trackingModes, CultureInfo.InvariantCulture));

			// never want to claim (save to disk) a payload if it is small, hence always probe for its size
			var isCompressible = trackingStream.AsMarkable().TryCompressToBase64String(PAYLOAD_SIZE_THRESHOLD, out var encodedCompression);
			if (isCompressible)
			{
		...
		/// <summary>
		/// Generates a unique file name according to the pattern expected by the tracking agent that moves locally claimed
		/// streams to the central claim store, that is from the <see cref="CheckInDirectory"/> to the <see
		...
		/// <list type="bullet">
		/// <item><description>
		/// <c>&lt;yyyyMMdd&gt;</c> is the current date, and denotes a subfolder in the claim store where all the message tracked
		/// at the same date will be store;
		/// </description></item>
		/// <item><description>
		...
		/// <summary>
		/// Whether a <see cref="FileStream"/> that has to be claimed into the <see cref="ClaimStore"/> requires to be checked in
		/// and out, or simply just saved into it.
		/// </summary>
		/// <remarks>
		/// <para>
		/// When a business message body is required to be checked in and out of the <see cref="ClaimStore"/>, its payload will
		/// first be claimed on the server's local file system (see <see cref="CheckInDirectory"/>), and then moved
		/// asynchronously to a network-shared file system (see <see cref="CheckOutDirectory"/>). For this reason, the business
		/// message body's payload will be replaced by a <see cref="Schemas.Xml.Claim.CheckIn"/> token message that will mutate into a <see
		/// cref="Schemas.Xml.Claim.CheckOut"/> token message as soon as it can be redeemed from the <see cref="ClaimStore"/>; that is when
		/// the the original business message body's payload has been made available in the <see cref="ClaimStore"/>.
		/// </para>
		/// <para>
		/// When a business message body is not required to be checked in and out of the <see cref="ClaimStore"/>, its payload
		/// will be directly saved in a shared location and will not need to be moved asynchronously to another location. For
		/// this reason, the business message body's payload will be replaced by an immutable <see cref="Schemas.Xml.Claim.Check"/> token
		/// message that can be readily redeemed from the <see cref="ClaimStore"/>.
		/// </para>
		/// </remarks>
		internal static bool RequiresCheckInAndOut => !string.Equals(Path.GetFullPath(CheckInDirectory), Path.GetFullPath(CheckOutDirectory), StringComparison.OrdinalIgnoreCase);

		/// <summary>
		/// Replace the message body's payload stream with either a <see cref="Schemas.Xml.Claim.Check"/> or a <see cref="Schemas.Xml.Claim.CheckIn"/>
		/// token message if its content has been assessed to be saved to disk while being tracked (see <see
		/// cref="SetupMessageBodyCapture"/>). Leave the message body's payload stream unaltered otherwise.
		/// </summary>
		/// <param name="message">
		/// The <see cref="IBaseMessage"/> whose message body's payload stream is going to be claimed and replace by a token
		/// message.
		/// </param>
		...
		/// </param>
		/// <remarks>
		/// The <see cref="IBaseMessage"/>'s <see cref="IBaseMessageContext"/> is also updated with the message type of the token
		/// message that is put in place of the actual message body's payload.
		/// </remarks>
		public virtual void Claim(IBaseMessage message, IResourceTracker resourceTracker)
		{
			...
			else
			{
				...
				System.IO.Stream tokenMessageStream;
				if (RequiresCheckInAndOut)
				{
					documentSpec = SchemaMetadata.For<Claim.CheckIn>().DocumentSpec;
					tokenMessageStream = ClaimFactory.CreateCheckIn(claimedMessageType, trackingStream.CaptureDescriptor.Data).AsStream();
				}
				else
				{
					documentSpec = SchemaMetadata.For<Claim.Check>().DocumentSpec;
					tokenMessageStream = ClaimFactory.CreateCheck(claimedMessageType, trackingStream.CaptureDescriptor.Data).AsStream();
				}
		...
		/// claimed to disk while being processed, that is if current payload is either one of the <see cref="Schemas.Xml.Claim.Check"/> or
		/// <see cref="Schemas.Xml.Claim.CheckOut"/> token messages. Leave the message body's payload stream unaltered otherwise.
		/// </summary>
		/// <param name="message">
		/// The <see cref="IBaseMessage"/> token message whose message body's payload stream is going to be restored to the
		/// payload that has been previously claimed.
		/// </param>
		/// <param name="resourceTracker">
		/// Pipeline's <see cref="IResourceTracker"/> to which to register the restored message body stream for later cleanup.
		/// </param>
		/// <remarks>
		/// The <see cref="TrackingStream"/> that will replace the <paramref name="message"/> stream with its previously captured
		/// original payload, will have its <see cref="TrackingStream.CaptureDescriptor"/> point to the previously
		...
		public virtual void Redeem(IBaseMessage message, IResourceTracker resourceTracker)
		{
			...
			if (messageType == SchemaMetadata.For<Claim.Check>().MessageType || messageType == SchemaMetadata.For<Claim.CheckOut>().MessageType)
			{
				var messageBodyCaptureDescriptor = message.BodyPart.AsMessageBodyCaptureDescriptor();
				if (_logger.IsDebugEnabled) _logger.Debug($"Performing redeem of claim check token '{messageBodyCaptureDescriptor.Data}'.");
			...
			else if (messageType == SchemaMetadata.For<Claim.CheckIn>().MessageType)
			{
				throw new InvalidOperationException($"Invalid token message, {SchemaMetadata.For<Claim.CheckIn>().RootElementName} token is not expected to be redeemed.");
			}
			else
			{
