...
namespace Be.Stateless.BizTalk
{
	namespace Factory
	{
		namespace Areas
		{
			...
			/// <c>Be.Stateless.ECommerce.Areas.Shopping.CustomerRegistration</c>,
			/// <c>Be.Stateless.ECommerce.Areas.Shopping.Invoicing</c>, and <c>Be.Stateless.ECommerce.Areas.Shopping.Shipping</c>
			/// cover 3 different business processes (<c>CustomerRegistration</c>, <c>Invoicing</c>, and <c>Shipping</c>) in the
			/// <c>Shopping</c> area if the <c>ECommerce</c> LOB/project.
			/// </para>
			/// <para>
			/// These tokens are of importance for the monitoring web site accompanying BizTalk Factory. An area named <see
			/// cref="Default"/> can be used when one does not wish to use areas to partition processes. The default area will not
