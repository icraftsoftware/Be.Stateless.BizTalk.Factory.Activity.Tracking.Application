...
ApplicationManifest -Name BizTalk.Factory.Activity.Tracking -Description 'BizTalk.Factory''s activity tracking application add-on for general purpose BizTalk Server development.' -Reference BizTalk.Factory -Build {
   ...
   SqlDeploymentScript -Path (Get-ResourceItem -Extension .sql -Name Create.BAMPrimaryImport.Objects) -Server $MonitoringServer -Variable @{
      BamOnlineWindowTimeLength = $BamOnlineWindowTimeLength
      BizTalkHostUserGroups     = $BizTalkHostUserGroup -join ';'
   }
   SqlUndeploymentScript -Path (Get-ResourceItem -Extension .sql -Name Drop.BAMPrimaryImport.Objects) -Server $MonitoringServer -Variable @{
      BizTalkHostUserGroups = $BizTalkHostUserGroup -join ';'
   }
   SqlDeploymentScript -Path (Get-ResourceItem -Extension .sql -Name Create.BizTalkServerOperator) -Server $ManagementServer -Variable @{
   ...
   SqlDeploymentScript -Path (Get-ResourceItem -Extension .sql -Name Create.BamTrackingActivitiesMaintenanceJob) -Server $MonitoringServer -Variable @{
