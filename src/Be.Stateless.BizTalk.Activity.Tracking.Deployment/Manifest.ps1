...
ApplicationManifest -Name BizTalk.Activity.Tracking -Description 'BizTalk.Factory''s activity model and tracking API for general purpose BizTalk Server development.' -Build {
   ...
   SqlDeploymentScript -Path (Get-ResourceItem -Extensions .sql -Name TurnOffGlobalTracking, CreateMonitoringObjects) -Server $ManagementServer -Variables @{ Test = 1 }
   SqlUndeploymentScript -Path (Get-ResourceItem -Extensions .sql -Name DropMonitoringObjects) -Server $ManagementServer -Variables @{ Test = 1 }
   SqlDeploymentScript -Path (Get-ResourceItem -Extensions .sql -Name CreateClaimCheckObjects) -Server $ProcessingServer -Variables @{ Test = 1 }
   SqlUndeploymentScript -Path (Get-ResourceItem -Extensions .sql -Name DropClaimCheckObjects) -Server $ProcessingServer -Variables @{ Test = 1 }
   ...
   SqlUndeploymentScript -Path (Get-ResourceItem -Extensions .sql -Name DropBizTalkServerOperator) -Server $ManagementServer -Variables @{ Test = 1 }
   ...
   SqlUndeploymentScript -Path (Get-ResourceItem -Extensions .sql -Name DropBamTrackingActivitiesMaintenanceJob) -Server $MonitoringServer -Variables @{ Test = 1 }
